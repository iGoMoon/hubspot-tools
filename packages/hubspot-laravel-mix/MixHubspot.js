...
class MixHubspot {
    ...
    boot() {
        ...
        try {
            require(this.context.resolve(
                '@hubspot/webpack-cms-plugins/HubSpotAutoUploadPlugin'
            ));
        } catch (e) {
    ...
    webpackPlugins() {
        let pluginsList = [
            !!this.Notifications
                ? this.components.UseCustomWebpackNotifier()
                : false,
    ...
    options = {
        ...
        component: (key = '', defaults = null) => {
            ...
            return typeof passed != 'object'
                ? passed
                : Object.assign({}, defaults, passed);
        },
    };

    components = {
        UseFieldsJs: () => {
            const { FieldsPlugin } = require(this.context.resolve(
        ...
        UseHubspotAutoUploader: () => {
            ...
            const CopyWebpackPlugin = require(this.context.resolve(
                'copy-webpack-plugin'
            ));
            const HubSpotAutoUploadPlugin = require(this.context.resolve(
                '@hubspot/webpack-cms-plugins/HubSpotAutoUploadPlugin'
            ));
                                ...
                                async compilation => {
                                    Object.entries(compilation.assets).forEach(
                                        ([path, data]) => {
                                            ...
                                            // Update Emitted
                                            compilation.emittedAssets.delete(
                                                path
                                            );
                                            compilation.emittedAssets.add(
                                                newPath
                                            );
                                        }
                                    );
        ...
        UseCustomWebpackNotifier: () => {
            const WebpackBuildNotifierPlugin = require(this.context.resolve(
        ...
        DisableMixTableOutput: config => {
            let BuildOutputPlugin = require(this.context.resolve(
                'laravel-mix/src/webpackPlugins/BuildOutputPlugin'
            ));
            config.plugins = config.plugins.filter(
                p => !(p instanceof BuildOutputPlugin)
            );
        },
    };
    ...
    get isAutoUpload() {
        return process.argv.some(arg => arg.includes('hsAutoUpload=true'));
    }

    get isHsSilent() {
        return process.argv.some(arg => arg.includes('hsSilent=true'));
    }
